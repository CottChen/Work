...
| 概念 | 说明 | 典型应用 |
|------|------|----------|
| **B-Tree** | 多路平衡查找树，每个节点都存数据 | MongoDB 索引、PostgreSQL 索引 |
| **B+Tree** | B-Tree 变体，数据只在叶子节点，叶节点有链表 | MySQL InnoDB、SQL Server |
| **LSM-Tree** | 日志结构合并树，顺序写 + 后台合并 | RocksDB、WiredTiger、HBase |
| **聚簇索引** | 数据与索引在一起，主键即数据位置 | MySQL InnoDB |
| **非聚簇索引** | 索引指向数据位置，额外一次查找 | MongoDB、MySQL 二级索引 |
...
| **索引结构** | B-Tree (非聚簇) | B+Tree (聚簇) | B-Tree + GIN/GiST | 无索引 (键即索引) |
| **数据存储** | LSM-Tree | B+Tree | Heap | 内存 |
...
### C.2 MongoDB 索引 vs MySQL 索引：常见误解澄清

**误解**："MongoDB 使用 B+Tree 存储数据"

**事实**：需要区分**索引结构**和**数据存储结构**
